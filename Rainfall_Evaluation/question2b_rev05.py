...
def gen_for_catchment(catchment, locations):
    ...
    del rad_c,det_c,fore_c,rad_cos_c,det_cos_c,for_cos_c
...
def evaluate_for_catchment(cats):
    ...
    for lead in range(3,max_lead+1,3):
        ...
        roc_auc_dic[variables[0]][c] = ROC(lib_ic['radar'], lib_ic[variables[0]]).roc_auc()
        progress_bar.update(1)   
        roc_auc_dic[variables[1]][c] = ROC(lib_ic['radar'], lib_ic[variables[1]]).roc_auc(90)
        progress_bar.update(1)   
        roc_auc_dic[variables[2]][c] = ROC(lib_cos['radar'], lib_cos[variables[2]]).roc_auc()
        progress_bar.update(1)   
        roc_auc_dic[variables[-1]][c] = ROC(lib_cos['radar'], lib_cos[variables[-1]]).roc_auc(90)
        progress_bar.update(1)    
...
def plot_curves(results, catchment_name):
    ...
    for res in list(results.keys()):
        ...
        plt.plot(leads, results[res]['ICOND2EPS'],'--',color = 'blue',label='Ensemble 90% | ICOND2' )
        
        plt.plot(leads, results[res]['COSMOD2'],'-',color = 'orange',label='Main run | COSMOD2' )
        plt.plot(leads, results[res]['COSMOD2EPS'],'--',color = 'orange',label='Ensemble 90% | COSMOD2' )
