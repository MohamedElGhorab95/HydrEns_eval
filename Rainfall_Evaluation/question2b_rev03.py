...
def gen_for_catchment(catchment, locations):
    ...
    rad_det = Observation(locations[3]).gen_observation_field()
    rad_ens = Observation(locations[4]).gen_observation_field()
...
def evaluate_for_catchment(cats):
    ...
    for lead in range(3,max_lead+1,3):
        ...
        roc_auc_dic[variables[0]][c] = ROC(lib_ic['radar'], lib_ic[variables[0]]).roc_auc(threshold)
        progress_bar.update(1)   
        roc_auc_dic[variables[1]][c] = ROC_totens(lib_ic['radar'], lib_ic[variables[1]],threshold).area
        progress_bar.update(1)   
        roc_auc_dic[variables[2]][c] = ROC(lib_cos['radar'], lib_cos[variables[2]]).roc_auc(threshold)
        progress_bar.update(1)   
        roc_auc_dic[variables[-1]][c] = ROC_totens(lib_coseps['radar'], lib_coseps[variables[-1]],threshold).area
        progress_bar.update(1)    
