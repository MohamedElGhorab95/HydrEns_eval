...
def load_event_file(leadtime):
    ...
    with open(source_file_path, 'r') as f:
        ...
        radar_file = netcdf+ 'fertig' + '/' + 'radRW_cosmod2eps.nc'
        # deter_file = netcdf+ str(leadtime) + '/' + '{}hour_icond2.nc'.format(leadtime)
        ensem_file = netcdf+ str(leadtime) + '/' + '{}hour_cosmod2eps.nc'.format(leadtime)
...
def gen_for_catchment(catchment, locations):
    ...
    rad_c = rad_c.avg_areal_prec().aggr_temporal(3)
...
def plot_curves(results):
    ...
    for res in results:
        ...
        ax.set_ylim(0,0.6) 
        ax.set_title("Performance of COSMOD2 Ensemble forecast  \n{}".format(regions[region]))
        ax.set_xlabel('Leadtime (hrs)')
        ax.set_ylabel('CRPS')
        
        plt.savefig("//vs-grp07.zih.tu-dresden.de/howa/work/students/Mohamed_Elghorab/results/CRPS_COSMOD2_{}".format(regions[region]), bbox_inches = 'tight')        
        ...
        plt.savefig("//vs-grp07.zih.tu-dresden.de/howa/work/students/Mohamed_Elghorab/results/RMSE_COSMOD2_{}".format(regions[region]), bbox_inches = 'tight')        
        ...
        plt.savefig("//vs-grp07.zih.tu-dresden.de/howa/work/students/Mohamed_Elghorab/results/Disc_COSMOD2_{}".format(regions[region]), bbox_inches = 'tight')        
