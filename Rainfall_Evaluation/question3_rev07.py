...
def calc_metric(observation, forecast , metric):
    ...
    if metric == 'Frequency bias':
        ...
        return (float(ROC_totens(observation, forecast,2.5).area))
...
def plot_spatial(list_of_dics, metric):
    ...
    for l in list_of_dics:
        ...
        for u in l.keys():
            ...
            plt.plot(leads, l[u][0],label='{}km\u00b2 |{}'.format(u,l[u][1]) )
        ...
        if metric   == 'Area under ROC curve': ax.set_ylim(0.65,1.0)
        elif metric == 'Accuracy'            : ax.set_ylim(90,100), ax.set_ylabel(metric+'(%)')
        elif metric == 'Frequency bias'      : ax.set_ylim(0.45,1.4)
...
# plot_spatial(df,'Area under ROC curve')
