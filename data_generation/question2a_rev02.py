...
def evaluate_for_catchment(cats):
    ...
    for lead in range(3,max_lead+1,3):
        ...
        roc_auc_dic['main_run'][c] = ROC(lib_cos['radar'], lib_cos['COSMOD2']).roc_auc()
        
        for x in list(variables.keys())[1:]:
            roc_auc_dic[x][c] = ROC(lib_coseps['radar'], lib_coseps[variables[x][0]]).roc_auc(quantile=variables[x][1])
