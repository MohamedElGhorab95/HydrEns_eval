...
def gen_leadtime_netcdf(events, netcdf_file, run_name):
    '''
    This function generates leadtime netcdfs from forecast files
    ...
    for lead in leadt:
        for ev in events.keys():
            ...
            la = data_low_level.latitude.values
            lo = data_low_level.longitude.values
        ...
        dataset_final.latitude.values = la
        dataset_final.longitude.values = lo
    return dataset_final
        # dataset_final.to_netcdf('C:/netCDFs/{}/{}hour_{}.nc'.format(lead,lead,run_name)).compute(scheduler='threads')
...
x = gen_leadtime_netcdf(events, '//vs-grp07.zih.tu-dresden.de/howa/work/students/Mohamed_Elghorab/netCDFs/fertig/icond2_rev.nc','icond2eps')





data_low_level = xr.open_dataset("C:/netCDFs/icond2_rev.nc",chunks='auto').isel(forecast_time=slice(0,100))
# data_low_level = data_low_level.rename({"time": "start_time", "forecast_time": "time"})

# data_low_level.latitude.values
# data_low_level.longitude.values


# t1 = data_low_level.sel(start_time=slice(events[1][0],events[1][1]))
...
# t2 = t1.isel(start_time= 1 +7).resample(time="{}H".format(3),offset=dt.timedelta(hours=3-1)).sum()    
# t2['time'] = t2.start_time + t2.time
