...
def evaluate_single(cname):
    print("Process started for catchment: {}".format(cname), flush=True)
    tic = time.time()
    results = evaluate_for_catchment(cname)
    print("Process finished for catchment: {}".format(cname), flush=True)
    print("Operation time for catchment {}: {} hours".format(cname,(time.time()-tic)/3600), flush=True)
