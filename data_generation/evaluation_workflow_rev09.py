...
def evaluate_for_catchment(cats):
    ...
    for lead in range(3,max_lead+1,3):
        ...
        for x in variables.keys():
            ...
            con_acc_dic[x][c] = CONT(library['radar'], library[variables[x][0]],threshold).acc(variables[x][1])*100
            ...
            con_csi_dic[x][c] = CONT(library['radar'], library[variables[x][0]],threshold).csi(variables[x][1])
    ...
    return {'Area under ROC curve':roc_auc_dic, 'Accuracy':con_acc_dic , 
            ...
            'F1 score':con_f1_dic, 'F2 score':con_f2_dic, 'CSI': con_csi_dic}
...
def plot_curves(results, catchment_name):
    ...
    for res in list(results.keys()):
        ...
        if res   == 'Area under ROC curve': ax.set_ylim(0.65,1.0)
        elif res == 'Accuracy'            : ax.set_ylim(90,100)
