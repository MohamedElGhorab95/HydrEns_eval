...
def gen_for_catchment(catchment, locations):
    ...
    return library, eps_c



def calc_metric(event_dictionary, metric, threshold = 3):
...
def evaluate_for_catchment(cats):
    ...
    for lead in range(3,max_lead+1,3):
        ...
        library, ens = gen_for_catchment(cats,files)
        ...
        rad, det = library['radar'], library['deterministic_run']
        del library, files
        
        # calculating the roc curves on their own
        roc_auc_dic['main_run'][c]= ROC(rad, det).roc_auc()
        del det
        roc_auc_dic['ensemble_q10'][c]= ROC(rad, ens).roc_auc(quantile=10)
        roc_auc_dic['ensemble_q25'][c]= ROC(rad, ens).roc_auc(quantile=25)
        roc_auc_dic['ensemble_q75'][c]= ROC(rad, ens).roc_auc(quantile=75)
        roc_auc_dic['ensemble_q90'][c]= ROC(rad, ens).roc_auc(quantile=90)
        roc_auc_dic['ensemble_mean'][c]= ROC(rad, ens).roc_auc(quantile='mean')
        del rad
