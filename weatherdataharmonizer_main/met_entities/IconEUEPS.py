...
class IconEUEPS(MetEntities):
    ...
    def __init__(self,
        ...
        self.nc_desc = vd.NcDimVarDescription(dim_time='time', dim_forecast='forecast_time', dim_lon='lon',
                                              ...
                                              var_eps='eps', var_data='iconeueps')

    def read_file(self, start_datetime, directory='./', dir_time_descriptor=None, forecast_hours=120, eps_member=None,
                  scale_factor=1, fill_value=np.nan, short=None, harmonize_dt=True):
        """
        Reading in all available files of an IconEUEPS dataset. If a file for a specific forecast time is not available
        yet, the function automatically stops reading in and delivers a proper datastructure.

        :param start_datetime: time to start reading in IconEUEPS files, it is used to build the filenames following the
            convention of the german weather service (DWD)
        :type start_datetime: datetime.datetime
        :param directory: directory with all IconEUEPS files from start_datetime
        ...
        :param forecast_hours: the number of forecast hours to be read in, default is 120 (max for IconEUEPS)
        :type forecast_hours: int, optional
        :param eps_member: list with numbers of eps members to read (0 to 39); if not given, take all 40 realisations
        :type eps_member: list, optional
        ...
        :param short: if int16 or int32, the large data variables are cast to numpy.int16/int32 to minimize memory
            usage; the user must pay attention to the scale_factor to include the necessary precision
        :type short: str, optional
        :param harmonize_dt: if True (default), harmonize possibly changing dt to one final dt equal to the time
            duration between the first two IconEU data matrices; the data at longer dt are evenly distributed to the
            shorter target dt
        :type harmonize_dt: bool
        """
        if short:
            self.short = short

        # consider changing time resolution in IconEUEPS
        available_hours = np.concatenate((np.arange(0, 49), np.arange(51, 73, 3), np.arange(78, 121, 6)))
        idx = available_hours <= forecast_hours
        available_hours = available_hours[idx]
        ...
        # get coordinates
        clon, clat = readIcon.get_lonlat(variant='eueps')

        # build filenames and read in data
        filenames = []
        icon_data_realis = []
        forecast_value = []
        for i_ct, i in enumerate(available_hours):
            filename = 'icon-eu-eps_europe_icosahedral_single-level_' + start_datetime.strftime('%Y%m%d%H') + '_' + \
                       '{:03d}'.format(i) + '_tot_prec.grib2.bz2'
            ...
            try:
                icon_data = readIcon.read_icon_d2(filename=filenames[i_ct], scale_factor=scale_factor,
                                                  fill_value=fill_value, variant='eueps', short=self.short)

                for realis in range(len(icon_data)):
                    if i_ct == 0:
                        icon_data_realis.append([icon_data[realis]])
                    else:
                        icon_data_realis[realis].append(icon_data[realis])

            except IOError:
                if i_ct > 0:
                    print(f'Warning: file {filename} not available - stopping at {filenames[i_ct - 1]}')
                else:
        ...
        for member in self.eps_member:
            ...
            icon_data_flattened = gt.accum_to_instantaneous_flattened([icon_data_realis[member]], fill_value=fill_value)
            ...
            if harmonize_dt:
                # harmonize dt for all time steps
                icon_data_flattened, forecast_value_final = gt.harmonize_time_step(icon_data_flattened, fill_value=fill_value, forecast_time=forecast_value)
            else:
                forecast_value_final = copy.copy(forecast_value)
            if self.forecast_value is None:
                # fill self.forecast_value only once
                forecast_description = vd.TimeDescription(units='minutes')

                self.forecast_value = LonLatTime(data=np.array(forecast_value_final), data_description=forecast_description)
            ...
            if self.short == 'int16':
                gr_data.data = np.empty((icon_data_flattened[0].data.size, len(icon_data_flattened)), dtype=np.short)
            if self.short == 'int32':
                gr_data.data = np.empty((icon_data_flattened[0].data.size, len(icon_data_flattened)), dtype=np.int32)
            else:
                gr_data.data = np.empty((icon_data_flattened[0].data.size, len(icon_data_flattened)))
            for i in range(len(icon_data_flattened)):
                # convert list entries to ndarray
                gr_data.data[:, i] = icon_data_flattened[i].data
            gr_data.data_description = vd.DataDescription(fill_value=fill_value, scale_factor=scale_factor,
                                                          long_name='IconEUEPS precipitation data',
    ...
    def export_netcdf(self, filename, data_format='f8', version='separated', institution=None, scale_factor_nc=1,
        ...
        """
        Export the relevant content of this IconEUEPS class to a new netcdf file. The function expects the same
        ...
        if gr_data_first.lon.data.ndim == 2:
        ...
        if coord_matrix:
            ...
            ncfile.createDimension(self.nc_desc.dim_coord, gr_data_first.lon.data.size)  # coordinate axis
        ...
        if coord_matrix:
            ...
            if separation:
                ...
                if num_eps > 1:
                    iconeueps = ncfile.createVariable(self.nc_desc.var_data, data_format,
            ...
            if separation:
                ...
                if num_eps > 1:
                    iconeueps = ncfile.createVariable(self.nc_desc.var_data, data_format,
                                                      (self.nc_desc.dim_time, self.nc_desc.dim_coord,
                                                       self.nc_desc.dim_forecast, self.nc_desc.dim_eps),
                                                      fill_value=gr_data_first.data_description.fill_value,
                                                      **data_kwargs)
                else:
                    # if only one eps member is available
        ...
        if separation:
            for realis in range(num_eps):
                vars()[variable_names[realis]].units = precipitation_units  # gr_data_first.data_description.units
                vars()[variable_names[realis]].long_name = gr_data_first.data_description.long_name
                vars()[variable_names[realis]].scale_factor_internal = scale_factor_internal  # gr_data_first.data_description.scale_factor
                vars()[variable_names[realis]].scale_factor = scale_factor_nc
        else:
            iconeueps.units = precipitation_units  # gr_data_first.data_description.units
            iconeueps.long_name = gr_data_first.data_description.long_name
            iconeueps.scale_factor_internal = scale_factor_internal  # gr_data_first.data_description.scale_factor
            iconeueps.scale_factor = scale_factor_nc
        ...
        if coord_matrix:
            ...
            if separation:
                ...
                if isinstance(self.gr_data, GeoReferencedData):
                    # if only one eps member is available
                    iconeueps[0, :, :, :] = st.gr_data_scaling(self.gr_data, scale_undo,
                    ...
                    for gr_data in self.gr_data:
                        ct = ct + 1
                        iconeueps[0, :, :, :, ct] = st.gr_data_scaling(gr_data, scale_undo,
            ...
            if separation:
                ...
                if isinstance(self.gr_data, GeoReferencedData):
                    # if only one eps member is available
                    iconeueps[0, :, :] = st.gr_data_scaling(self.gr_data, scale_undo,
                                                            self.gr_data.data_description.scale_factor, scale_factor_nc)
                else:
                    ct = -1
                    for gr_data in self.gr_data:
                        ct = ct + 1
    ...
    def export_netcdf_append(self, filename):
        ...
        if separation:
            ...
            if self.nc_desc.var_data not in ncfile.variables:
                raise Exception(f'data variables are not appropriate in {filename}')
            if num_eps > 1 and (self.nc_desc.dim_eps not in ncfile.dimensions or self.nc_desc.var_eps not in ncfile.variables):
                raise Exception(f'either dimensions or variables are not appropriate in {filename}')
        ...
        if coord_matrix:
            ...
            if ncfile[self.nc_desc.var_lon].size != gr_data_first.lon.data.size:
                raise Exception(f'the data shape in {filename} differs from object shape')
