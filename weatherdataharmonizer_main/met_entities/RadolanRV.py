...
class RadolanRV(MetEntities):
    ...
    def __init__(self,
        ...
        self.nc_desc = vd.NcDimVarDescription(dim_time='time', dim_forecast='forecast_time', dim_lon='lon',
                                              ...
                                              var_lon='longitude', var_lat='latitude', var_data='radolanrv')
    ...
    def read_file(self, filename=None, start_datetime=None, directory='./', dir_time_descriptor=None, scale_factor=1,
        ...
        :param filename: RV filename typically in the form DE1200_RVyymmddHHMM.tar.bz2
        :type filename: str, optional
        :param start_datetime: time to start reading in RadolanRV files, it is used to build the filenames following the
        ...
        if filename is None:
            if dir_time_descriptor is not None:
                ...
                dir_time = ''
            filename = os.path.join(directory, dir_time, 'DE1200_RV' + start_datetime.strftime('%y%m%d%H%M') + '.tar.bz2')
        if not os.path.exists(filename):
            raise ForecastFileNotAvailable(f'forecast file {filename} not available - import not successful')

        print(f'load {filename}')

        with tarfile.open(filename, 'r:bz2') as fid_tar:
            # the tar.bz2 file contains all single radolan formatted files
            fc_files = fid_tar.getnames()

            rd_data = []
            for fc_file in fc_files:
                file_content_binary = fid_tar.extractfile(fc_file).read()
                rd_data.append(readRadolan.read_radolan_data(file_content_binary, scale_factor=scale_factor,
                                                             fill_value=fill_value, short=self.short))
            # collect data from all tar collected files the same way as RadvorRQ or IconD2 data
            num_fc = len(rd_data)
            forecast_value = np.empty(num_fc)
            if self.short:
                self.gr_data.data = np.empty([rd_data[0].metadata.nrows, rd_data[0].metadata.ncols, num_fc], dtype=np.short)
            else:
                self.gr_data.data = np.empty([rd_data[0].metadata.nrows, rd_data[0].metadata.ncols, num_fc])
            for i in range(num_fc):
                forecast_value[i] = rd_data[i].metadata.prediction_time
                self.gr_data.data[:, :, i] = rd_data[i].data
            self.gr_data.data_description = vd.DataDescription(fill_value=fill_value, scale_factor=scale_factor,
                                                               long_name='RadolanRV precipitation data',
                                                               units=f'{1/scale_factor} * mm/5min',
                                                               time_note=f'start at {rd_data[0].metadata.datum_iso}')
            ...
            self.gr_data.lon, self.gr_data.lat = readRadolan.get_lonlat(rd_data[0].metadata.format_version,
                                                                        grid_variant='de1200')
            time_value = 0
            time_unit = f"hours since {rd_data[0].metadata.datum_iso}"
    ...
    def export_netcdf(self, filename, data_format='f8', institution=None, scale_factor_nc=1, scale_undo=False,
        ...
        radolanrv = ncfile.createVariable(self.nc_desc.var_data, data_format,
        ...
        radolanrv.units = precipitation_units
        radolanrv.long_name = self.gr_data.data_description.long_name
        radolanrv.scale_factor_internal = scale_factor_internal
        radolanrv.scale_factor = scale_factor_nc
        ...
        radolanrv[0, :, :, :] = st.gr_data_scaling(self.gr_data, scale_undo, self.gr_data.data_description.scale_factor,
                                             scale_factor_nc)
