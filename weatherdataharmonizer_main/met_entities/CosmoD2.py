...
class CosmoD2(MetEntities):
    ...
    def read_file(self, start_datetime, directory='./', dir_time_descriptor=None, forecast_hours=27, scale_factor=1,
        ...
        for i in range(forecast_hours + 1):
            filename = 'cosmo-d2_germany_rotated-lat-lon_single-level_' + start_datetime.strftime('%Y%m%d%H') + '_' + \
            ...
            try:
                cosmo_data.append(readCosmo.read_cosmo_d2(filename=filenames[i], scale_factor=scale_factor,
        ...
        cosmo_data_flattened = gt.accum_to_instantaneous_flattened(cosmo_data, fill_value=fill_value)
        ...
        if self.short == 'int16':
            self.gr_data.data = np.empty((cosmo_data_flattened[0].data.shape + (len(cosmo_data_flattened),)), dtype=np.short)
        elif self.short == 'int32':
            self.gr_data.data = np.empty((cosmo_data_flattened[0].data.shape + (len(cosmo_data_flattened),)), dtype=np.int32)
        else:
            self.gr_data.data = np.empty((cosmo_data_flattened[0].data.shape + (len(cosmo_data_flattened),)))
        for i in range(len(cosmo_data_flattened)):
            # convert list entries to ndarray
            self.gr_data.data[:, :, i] = cosmo_data_flattened[i].data
        self.gr_data.data_description = vd.DataDescription(fill_value=fill_value, scale_factor=scale_factor,
                                                           long_name='CosmoD2 precipitation data',
    ...
    def export_netcdf(self, filename, data_format='f8', institution=None, scale_factor_nc=1, scale_undo=False,
        ...
        cosmod2 = ncfile.createVariable(self.nc_desc.var_data, data_format,
        ...
        cosmod2.units = precipitation_units
        cosmod2.long_name = self.gr_data.data_description.long_name
        cosmod2.scale_factor_internal = scale_factor_internal
        cosmod2.scale_factor = scale_factor_nc

        # write data to variables
        lon[:, :] = self.gr_data.lon.data
        lat[:, :] = self.gr_data.lat.data
        cosmod2[0, :, :, :] = st.gr_data_scaling(self.gr_data, scale_undo,
