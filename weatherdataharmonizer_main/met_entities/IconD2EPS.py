...
class IconD2EPS(MetEntities):
    ...
    def __init__(self,
        ...
        self.nc_desc = vd.NcDimVarDescription(dim_time='time', dim_forecast='forecast_time', dim_lon='lon',
                                              ...
                                              var_eps='eps', var_data='icond2eps')

    def read_file(self, start_datetime, directory='./', dir_time_descriptor=None, forecast_hours=48, eps_member=None,
                  scale_factor=1, fill_value=np.nan, short=None):
        """
        Reading in all available files of an IconD2EPS dataset. If a file for a specific forecast time is not available
        yet, the function automatically stops reading in and delivers a proper datastructure.

        :param start_datetime: time to start reading in IconD2EPS files, it is used to build the filenames following the
            convention of the german weather service (DWD)
        :type start_datetime: datetime.datetime
        :param directory: directory with all IconD2EPS files from start_datetime
        ...
        :param forecast_hours: the number of forecast hours to be read in, default is 48 (max for IconD2EPS)
        ...
        # get coordinates
        clon, clat = readIcon.get_lonlat(variant='d2eps')
        ...
        for i in range(forecast_hours + 1):
            filename = 'icon-d2-eps_germany_icosahedral_single-level_' + start_datetime.strftime('%Y%m%d%H') + '_' + \
            ...
            try:
                icon_data = readIcon.read_icon_d2(filename=filenames[i], scale_factor=scale_factor,
        ...
        for member in self.eps_member:
            ...
            gr_data.data_description = vd.DataDescription(fill_value=fill_value, scale_factor=scale_factor,
                                                          long_name='IconD2EPS precipitation data',
    ...
    def export_netcdf(self, filename, data_format='f8', version='separated', institution=None, scale_factor_nc=1,
        ...
        """
        Export the relevant content of this IconD2EPS class to a new netcdf file. The function expects the same
        ...
        if coord_matrix:
            ...
            if separation:
                ...
                if num_eps > 1:
                    icond2eps = ncfile.createVariable(self.nc_desc.var_data, data_format,
        ...
        if separation:
            ...
            icond2eps.units = precipitation_units  # gr_data_first.data_description.units
            icond2eps.long_name = gr_data_first.data_description.long_name
            icond2eps.scale_factor_internal = scale_factor_internal  # gr_data_first.data_description.scale_factor
            icond2eps.scale_factor = scale_factor_nc
        ...
        if coord_matrix:
            ...
            if separation:
                ...
                if isinstance(self.gr_data, GeoReferencedData):
                    # if only one eps member is available
                    icond2eps[0, :, :, :] = st.gr_data_scaling(self.gr_data, scale_undo,
                    ...
                    for gr_data in self.gr_data:
                        ct = ct + 1
                        icond2eps[0, :, :, :, ct] = st.gr_data_scaling(gr_data, scale_undo,
