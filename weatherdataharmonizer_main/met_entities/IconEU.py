...
class IconEU(MetEntities):
    ...
    def read_file(self, start_datetime, directory='./', dir_time_descriptor=None, forecast_hours=120, scale_factor=1,
                  fill_value=np.nan, short=None, harmonize_dt=True):
        """
        Reading in all available files of an IconEU dataset. If a file for a specific forecast time is not available
        yet, the function automatically stops reading in and delivers a proper datastructure.

        :param start_datetime: time to start reading in IconEU files, it is used to build the filenames following the
            convention of the german weather service (DWD)
        :type start_datetime: datetime.datetime
        :param directory: directory with all IconEU files from start_datetime
        ...
        :param forecast_hours: the number of forecast hours to be read in, default is 120 (max for IconEU)
        ...
        # consider changing time resolution in IconEU
        if start_datetime.hour in [0, 6, 12, 18]:
            available_hours = np.concatenate((np.arange(0, 79), np.arange(81, 121, 3)))
        else:
            available_hours = np.concatenate((np.arange(0, 31), np.arange(36, 49, 6)))
        idx = available_hours <= forecast_hours
        available_hours = available_hours[idx]

        # get coordinates
        clon, clat = readIcon.get_lonlat(variant='eu')
        ...
        for i_ct, i in enumerate(available_hours):
            filename = 'icon-eu_europe_regular-lat-lon_single-level_' + start_datetime.strftime('%Y%m%d%H') + '_' + \
            ...
            try:
                icon_data.append(readIcon.read_icon_d2(filename=filenames[i_ct], scale_factor=scale_factor,
                                                       fill_value=fill_value, variant='eu', short=self.short))
            except IOError:
                if i_ct > 0:
        ...
        if harmonize_dt:
            ...
            icon_data_flattened, forecast_value = gt.harmonize_time_step(icon_data_flattened, fill_value=fill_value,
        ...
        if self.short == 'int16':
            self.gr_data.data = np.empty((icon_data_flattened[0].data.shape + (len(icon_data_flattened),)), dtype=np.short)
        elif self.short == 'int32':
            self.gr_data.data = np.empty((icon_data_flattened[0].data.shape + (len(icon_data_flattened),)), dtype=np.int32)
        else:
            self.gr_data.data = np.empty((icon_data_flattened[0].data.shape + (len(icon_data_flattened),)))
        ...
        self.gr_data.data_description = vd.DataDescription(fill_value=fill_value, scale_factor=scale_factor,
                                                           long_name='IconEU precipitation data',
    ...
    def export_netcdf(self, filename, data_format='f8', institution=None, scale_factor_nc=1, scale_undo=False,
        ...
        if coord_matrix:
            ...
            iconeu = ncfile.createVariable(self.nc_desc.var_data, data_format,
        ...
        iconeu.units = precipitation_units
        iconeu.long_name = self.gr_data.data_description.long_name
        iconeu.scale_factor_internal = scale_factor_internal
        iconeu.scale_factor = scale_factor_nc

        # write data to variables
        if coord_matrix:
            lon[:, :] = self.gr_data.lon.data
            lat[:, :] = self.gr_data.lat.data
            iconeu[0, :, :, :] = st.gr_data_scaling(self.gr_data, scale_undo,
                                                    self.gr_data.data_description.scale_factor,
                                                    scale_factor_nc)
        else:
            lon[:] = self.gr_data.lon.data
            lat[:] = self.gr_data.lat.data
            iconeu[0, :, :] = st.gr_data_scaling(self.gr_data, scale_undo, self.gr_data.data_description.scale_factor,
                                                 scale_factor_nc)
