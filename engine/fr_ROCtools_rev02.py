...
class ROC(object):
    ...
    def roc_auc(self, quantile= None):
        ...
        try:       
            ...
            # sorting the data
            pofd_df = pofd_df.sort_values()
            pod_df = pod_df.reindex(pofd_df.index)
            ...
            # create the dataframes to store all curves 
            pod_df = pd.DataFrame(pod_kl)
            pofd_df = pd.DataFrame(pofd_kl)
            
            # sorting the data
            pofd_df = pofd_df.sort_values(by=0)
            pod_df = pod_df.reindex(pofd_df.index)
